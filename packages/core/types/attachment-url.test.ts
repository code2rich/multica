...
describe("contentReferencesAttachment", () => {
  ...
  // Regression — issue DESCRIPTION editor binding.
  //
  ...
  // couldn't resolve it to a freshly-signed `download_url`. `contentReferencesAttachment`
  // matches the absolute-host download URL via its stable-path substring, so
  // the attachment now binds the same way comments do.
