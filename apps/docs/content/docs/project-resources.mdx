...
`local_directory` resources are managed through the CLI. The web app does not expose a native folder picker, so you supply the daemon ID and path yourself:
...
Different daemons are independent — a shared project can have one `local_directory` per teammate's machine, each binding the project to a different folder on a different host. When the daemon claims a task, it picks the row that matches its own ID and ignores the rest.
